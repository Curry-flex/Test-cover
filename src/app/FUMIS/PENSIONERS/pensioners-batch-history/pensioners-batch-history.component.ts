...
@Component({
  selector: 'app-pensioners-batch-history',
  templateUrl: './pensioners-batch-history.component.html',
  styleUrls: ['./pensioners-batch-history.component.scss']
})
export class PensionersBatchHistoryComponent extends SharedClassComponent implements OnInit {
  ...
  getList() {
    ...
    const endDatestring =  new Date(this.pensionersBatchHistorytForm.get('endDate').value).getFullYear() + "-"+  ("0"+(new Date(this.pensionersBatchHistorytForm.get('endDate').value).getMonth()+1)).slice(-2) + "-" +("0" + new Date(this.pensionersBatchHistorytForm.get('endDate').value).getDate()).slice(-2);
      ...
      const data = {
        "requestType": "PENSION_BATCH_HISTORY",
